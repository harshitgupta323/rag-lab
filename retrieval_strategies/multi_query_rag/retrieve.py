...
from config import VECTOR_DB_DIR, LOG_DIR, EMBEDDING_MODEL, RETRIEVAL_TOP_K_PER_QUERY, FINAL_TOP_K, RRF_K
...
def retrieve_for_query(query: str):
    """Retrieve documents for a single query variant."""

    logger.info("Retrieving documents | top_k=%d", RETRIEVAL_TOP_K_PER_QUERY)
    logger.debug("Retrieval query: %s", query)

    vector_store = get_vector_store()

    documents = vector_store.similarity_search(query, k=RETRIEVAL_TOP_K_PER_QUERY)
...
def reciprocal_rank_fusion(result_lists, k=RRF_K):
    ...
    logger.info("RRF produced %d unique documents", len(fused_documents))

    return fused_documents


def retrieve_documents(query_variants):
    """Retrieve and fuse documents for multiple query variants."""

    logger.info("========== MULTI-QUERY RETRIEVAL STARTED ==========")
    logger.info("Processing %d query variants", len(query_variants))

    result_lists = []

    for index, query in enumerate(query_variants, start=1):
        logger.info("Retrieving for query variant %d/%d", index, len(query_variants))
        documents = retrieve_for_query(query)
        result_lists.append(documents)

    fused_documents = reciprocal_rank_fusion(result_lists)
